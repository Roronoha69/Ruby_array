...
def total(longueur)
    print "le total de handle est : "
    puts longueur.length
end

def court(data)
    print "le blaze le plus court : "
    puts data.min_by{|x| x.size}
end


def t_5(data)
    taille = data.find_all{|x| x.size == 6}
    puts "Les blazes de 5 caractere : " 
    taille.each {|x| puts "#{x[1..-1]}"}
end

def majuscule(data)
    print "les blazes a majuscule"
    puts data.select{ |x| x[1]==x[1].capitalize}
end

def trie_a(data)
   puts data.sort_by{ |x| x[1]}
end

def trie_l(data)
    puts data.sort_by{|x| x.size}
end
...
def repartition(data)
    ...
    data = data.sort_by{|x| x.size}

    current_size = data[0]
    counter = 1

    data.each{|j| 
    if j.size == current_size
    counter += 1
    else
    puts "Il y a #{counter} mots de #{current_size} caracteres"
    current_size= j.size
    counter =1
    end}
